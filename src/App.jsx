...
import PropsCheatSheet from './PropsCheatSheet';
  ...
  const generatePicks = useCallback(async ()=>{
    ...
    try {
      ...
      if (gamesData.success && gamesData.games?.length) {
        ...
        const statPromises = gamesData.games.slice(0,8).map(g=>
...
Rules: ${report.rules?.join(' | ')}`,
  ...
  const TABS=['dashboard','ai','groq','paste','mine','props','lessons','log'];
  const TLABELS={dashboard:'📊 Dash',ai:'🤖 Claude',groq:'🧠 Groq',paste:'📋 Paste',mine:'📈 My Picks',props:'⚾ Props',lessons:`🎓 (${state.lessons.length})`,log:'🪵 Log'};
                        ...
                        <div key={`${key}-${existingPick?.id||'none'}`} style={{marginBottom:10,padding:'12px 14px',background:'rgba(5,8,16,0.8)',borderRadius:10,border:`1px solid ${existingPick?'rgba(139,92,246,0.4)':'#1e293b'}`}}>
                          ...
                          {!existingPick&&!isAnalyzing&&(
                            <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                              ...
                              {/*NRFI disabled - no dedicated model yet*/}
                            </div>
                          )}
